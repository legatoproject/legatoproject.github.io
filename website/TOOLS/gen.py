...
def file_dispatch(dir, filename, subdir=None, only_in_subdir=True):
    ...
    for i in range(MAX_DEPTH):
        try:
            ...
            already.append(dir)
            # print "Not in " + dir
            if os.path.samefile(os.path.abspath(dir),
                                os.path.abspath(src_dir)):  # if we can't find it in src_dir, don't ascend further
...
def format_nav_link(template, json_link, target_file):
    return template.format(json_link["href"],
                           ' class="link-selected"' if match_link(json_link["href"], target_file) else "",
                           json_link["title"])
...
def gen_sidemenu(dir, filename, contents):
    ...
    outerHTML = render_file(join(templatedir, "_templates/", jsondata["template"]), dir).format(innerHTML,
                                                                                                jsondata.get("title",
...
def gen_autotopmenu(dir, filename, contents):
    ...
    x = cat_for_link(filename)
    if x is not None:
        contextname, my_cat = x
        context = flat_contexts[contextname]
        for cat in context:
            output += innerHTML.format(context[cat][0], ' class="link-selected"' if my_cat == cat else "",
                                       cat)
...
def render(filepath, rel_dir, contents):
    ...
    for k in funcs.keys():
        if k in contents:
            ...
            contents = contents.replace(k, f(rel_dir, filename,
                                             contents).encode())  # take the appropriate function and replace the marker with its output
    return contents
...
if __name__ == "__main__":
    ...
    # list of contexts. each context has categories which are lists of pages.

    flat_contexts = {page['label']: {cat['label']: get_tree_hrefs(cat) for cat in page['children']} for page in contexts}

    for ctx in contexts:
        for cat in ctx['children']:
            pass
            with open(join(tocdir, ctx['label'] + " " + cat['label'] + ".json"), "w") as f:
                json.dump(cat, f)
    ...
    for dir, subdirs, files in os.walk(join(os.getcwd(), src_dir)):
        ...
        for file in files:
            ...
            if filepath.endswith(".html"):
                split = file.split(".")
                if len(
                        split) > 2:  # if the file is like foo.bar.html, we wanna insert the file into the bar.html template
                    ...
                    with open(filepath) as f:
                        contents = rendered_template.replace("{content}",
                                                             f.read())  # insert content into rendered template
