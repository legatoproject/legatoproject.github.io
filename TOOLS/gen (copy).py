...
def gen_topmenu(dir, filename, contents, page):
    return gen_nav("topmenu.json",join(dir,filename))


def gen_header(dir, filename, contents, page):
    return gen_nav("header.json",join(dir,filename))

def gen_sidemenu(dir, filename, contents, page):
    ...
    outerHTML = render_file(join(templatedir,"_templates/", jsondata["template"]), dir, page).format(innerHTML, jsondata.get("title","")) # RECURSION HAPPENS HERE!!!!!
    return outerHTML
...
def gen_tocfile(dir, filename, contents, page):
    x = cat_for_link(filename, page = page)
    if x is None:
        print filename + " doesn't seem to be in the toc."
        return ""
    else:
        return "/resources/tocs/" + x[0]['label'] +"_" + x[1] + ".json"
...
def gen_autotopmenu(dir, filename, contents, page):
    ...
    """
    output = ""
    c = cat_for_link(filename, page)
    if c is not None:
        my_cat = cat_for_link(filename, page)[1]
...
def render_file(filepath, rel_dir, page):
    with open(filepath, 'r') as f:
        contents = f.read()
    return render(filepath,rel_dir,contents, page)

def render(filepath, rel_dir, contents, page):
    ...
    for k in funcs.keys():
        if k in contents:
            ...
            op = f(rel_dir,filename , contents, page)
            if op is None:
                pass
            contents = contents.replace(k,op.encode()) # take the appropriate function and replace the marker with its output
    return contents
...
def cat_for_link(href, page=None):
    if page is not None:
        cats = page['children']
...
if __name__ == "__main__":
    ...
    pages = split_toc_into_categories(join(src_dir, "toc.json"))
    cats = page['children']
    ...
    flat_cats = {cat['label']:get_tree_hrefs(cat) for cat in cats}
    for cat in cats:
        with open(join(tocdir, page['label'] + "_" + cat['label'] + ".json"), "w") as f:
            json.dump(cat, f)
    ...
    for dir, subdirs, files in os.walk(join(os.getcwd(), src_dir)):
        ...
        for file in files:
            ...
            if filepath in already:
                continue
            elif cat_for_link(file, page=page) is not None:
                already.add(filepath)
            if filepath.endswith(".html"):
                ...
                if len(split) > 2:  # if the file is like foo.bar.html, we wanna insert the file into the bar.html template
                    ...
                    rendered_template = render(resulting_filepath, rel_dir, tp, page) # render elements in template
                    with open(filepath) as f:
                    ...
                    contents = render_file(filepath,rel_dir, page)
                    savefile=file
